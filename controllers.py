...
def process_card(card, scale=10, split=''):
    ...
    Processes and updates all additional fields (cmc, loyalty, tilde rules) on a card that is already populated with
    basic data like mana cost, name, oracle rules, etc.
    '''
    ...
    typ, subtyp = parsers.types(card.type, split=split)
    card.processed_types = typ
    card.processed_subtypes = subtyp
